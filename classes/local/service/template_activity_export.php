...
class template_activity_export {
    ...
    public static function parameters_for(cm_info $cm): array {
        ...
        if ($cm->modname === 'scorm') {
            return self::scorm_parameters($cm);
        }
        return [
            'name' => $cm->name,
     ...
     *   scorm_popup_options()), and never as the packed string.
     ...
     * @return array
     */
    private static function scorm_parameters(cm_info $cm): array {
        global $CFG, $DB;
        ...
        $parameters = array_merge(
            self::scorm_settings_columns($scorm),
            self::scorm_popup_options($scorm),
            [
                'name' => $cm->name,
                'section' => (int) $cm->sectionnum,
                'intro' => $scorm->intro ?? '',
                'gradepass' => self::scorm_grade_pass($cm),
                // Always reported, null included: an absent key and a package
                // this activity does not have mean different things to the
                // service, and only one of them is true here.
                'moldscorm' => self::scorm_mold_package($cm, $scorm),
            ]
        );
     ...
     * @return array<string, int>
     */
    private static function scorm_popup_options($scorm): array {
        global $CFG;
     ...
     * scorm_popup_options() expands it into replace it.
     *
     ...
     * @return array
     */
    private static function scorm_settings_columns($scorm): array {
        $settings = [];
        foreach (self::SCORM_SETTINGS_COLUMNS as $field) {
            if (isset($scorm->$field)) {
                $settings[$field] = $scorm->$field;
            }
        }
        return $settings;
    }
     ...
     * @return float
     */
    private static function scorm_grade_pass(cm_info $cm): float {
        global $CFG;
     ...
     * @return array|null
     */
    private static function scorm_mold_package(cm_info $cm, $scorm): ?array {
        if ((string) ($scorm->scormtype ?? '') !== SCORM_TYPE_LOCAL) {
            return null;
        }

        $fs = get_file_storage();
        $files = $fs->get_area_files($cm->context->id, 'mod_scorm', 'package', 0, 'id', false);
        $file = reset($files);
        if (!$file) {
            return null;
        }
        ...
        try {
            ...
            $indexhtml = $zip->getFromName('index.html');
            $zip->close();
        } finally {
            @unlink($temppath);
        }

...
- [ ] **Redesign the "allowed activity types" picker so it stops being a long, hard-to-scan list of checkboxes.** Today it lists a checkbox for every activity type installed on the whole site, which is unwieldy — build a proper, purpose-built control for this instead (e.g. a compact multi-select with search/filtering) so finding and toggling a type doesn't mean scanning dozens of rows.
- [ ] **Only offer activity types the AI generation service can actually produce**, not every type installed on the site. Today this list is populated from every activity type Moodle has installed, regardless of whether the AI service knows how to generate that kind of content at all — restrict it to the set the AI service's own content contract actually supports, so an admin can never allow a type here that would just silently fail (or need manual authoring) when a course is generated.

...
Both course 397 and 398 use Moodle's `grid` course format (confirmed via `mdl_course.format` and `mdl_course_format_options`), which shows each section as a picture tile on the course's main page (`sectionimage_filemanager`, one per section, stored as a course-format option — separate from the in-content Label banner activity). `template_course_builder_service.php` already copies the course's `format` to the new course (line 273-274), so the *option that a course uses grid* survives — but nothing sets a section's `sectionimage_filemanager` for a section this feature creates fresh (i.e. any module beyond the template's original count, or any new section format_grid itself auto-creates). A generated course could plausibly end up with, say, modules 1-5 showing their grid picture and a newly-added module 6 showing none — the exact kind of visual inconsistency this client has already complained about once this session (see the banner work earlier). Phase 1 below treats deciding what happens to that per-section picture, for a section that didn't exist in the original template, as part of the same configuration decision as everything else — not an afterthought.

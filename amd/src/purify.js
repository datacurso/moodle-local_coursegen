...
/**
 * DOMPurify wrapper used by Course AI.
 *
 * Sanitizes the HTML produced from the model's Markdown before it reaches the
 * page. Vendored rather than taken from theme_boost/bootstrap/tools/sanitizer:
 * that file is a theme's private copy of Bootstrap's internals, it differs
 * between the themes installed on one site, and its allow-list parameter is
 * still Bootstrap 4's `whiteList` — renamed in Bootstrap 5, which would leave
 * the allow-list empty and silently strip every tag.
 *
 * The distribution build is kept unminified: this is amd/src, so it is the
 * source, and Grunt writes the minified copy the browser loads into amd/build.
 * The only edit is naming the AMD module, which Grunt then rewrites to
 * local_coursegen/purify.
 *
 * The first ~130 lines are not DOMPurify: _arrayLikeToArray, _slicedToArray and
 * the rest are Babel's helpers for compiling destructuring down to ES5, and
 * every published build carries them. DOMPurify itself is written in
 * TypeScript, which npm does not ship. Start reading at the constants — the
 * allow-lists and IS_ALLOWED_URI are where the decisions live.
 *
 * @module     local_coursegen/purify
 ...
 * Vendored from https://github.com/cure53/DOMPurify (dist/purify.js, v3.4.13).
 */
/*! @license DOMPurify 3.4.13 | (c) Cure53 and other contributors | Released under the Apache license 2.0 and Mozilla Public License 2.0 | github.com/cure53/DOMPurify/blob/3.4.13/LICENSE */

(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
  typeof define === 'function' && define.amd ? define("purify", factory) :

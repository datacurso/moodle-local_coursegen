...
/**
 * Unit tests for imscp_parameters — provider failure must not abort module creation.
 *
 ...
 */
final class imscp_parameters_test extends \advanced_testcase {
    /**
    ...
    public function test_provider_failure_degrades_gracefully(): void {
        ...
        $out = (new imscp_parameters($params))->get_parameters();
        $events = $sink->get_events();
        $sink->close();

        $this->assertDebuggingCalledCount(1);
        $this->assertFalse(isset($out->package));
        $this->assertSame('AI imscp', $out->name);
        ...
        $event = reset($skipped);
        $this->assertSame('imscp', $event->other['modname']);
    ...
    public function test_skip_event_uses_course_context_when_known(): void {
        ...
        $sink = $this->redirectEvents();
        (new imscp_parameters($params))->get_parameters();
        $events = $sink->get_events();
        $sink->close();
        $this->resetDebugging();
    ...
    public function test_missing_package_info_throws(): void {
        ...
        (new imscp_parameters((object) ['mod_settings' => []]))->get_parameters();
    }
}

...
/**
 * Unit tests for scorm_parameters — provider failure must not abort module creation.
 *
 ...
 */
final class scorm_parameters_test extends \advanced_testcase {
    /**
    ...
    public function test_provider_failure_degrades_gracefully(): void {
        ...
        $out = (new scorm_parameters($params))->get_parameters();
        $events = $sink->get_events();
        $sink->close();

        $this->assertDebuggingCalledCount(1);
        $this->assertFalse(isset($out->packagefile));
        $this->assertSame('AI scorm', $out->name);
        ...
        $event = reset($skipped);
        $this->assertSame('scorm', $event->other['modname']);
    ...
    public function test_skip_event_uses_course_context_when_known(): void {
        ...
        $sink = $this->redirectEvents();
        (new scorm_parameters($params))->get_parameters();
        $events = $sink->get_events();
        $sink->close();
        $this->resetDebugging();
    ...
    public function test_missing_package_info_throws(): void {
        ...
        (new scorm_parameters((object) ['mod_settings' => []]))->get_parameters();
    }
}
